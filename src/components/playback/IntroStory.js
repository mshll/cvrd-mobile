...
const NUM_LINES = 3; // Reduced number of lines
...
export function IntroStory() {
  ...
  const linesProgress = Array.from({ length: NUM_LINES }, () => useSharedValue(0));
  ...
  const LINE_LENGTH = SCREEN_WIDTH * 0.2; // Slightly longer lines
  const LINE_THICKNESS = 4; // Thicker lines
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      // Animate lines sequentially after bars have stabilized
      linesProgress.forEach((line, index) => {
        line.value = withDelay(
          3200 + index * 300, // Start after bars have completed rotation (3200ms = total time of bar animation)
          withTiming(1, { duration: 800 }) // Slower line animation
        );
      });
    }, 1000);
  ...
  // Create animated styles for each diagonal line
  const lineStyles = linesProgress.map((progress, index) => {
    return useAnimatedStyle(() => {
      const lineOpacity = interpolate(progress.value, [0, 0.3, 0.7, 1], [0, 1, 1, 0], Extrapolate.CLAMP);

      // Calculate movement along -45 degree angle
      const distance = LINE_LENGTH * 1.4; // Slightly longer distance to account for diagonal movement
      const translateX = interpolate(progress.value, [0, 1], [-distance, distance], Extrapolate.CLAMP);
      const translateY = interpolate(progress.value, [0, 1], [distance, -distance], Extrapolate.CLAMP);

      // Base vertical offset for staggering
      const baseTranslateY = -USABLE_HEIGHT * 0.2 + index * (USABLE_HEIGHT * 0.2);
      ...
      return {
        ...
        backgroundColor: colors.primary,
        opacity: lineOpacity,
        transform: [{ translateX }, { translateY: baseTranslateY + translateY }, { rotate: '-45deg' }],
      };
    });
  });
      ...
      <Animated.View style={[{ width: CONTAINER_SIZE, height: USABLE_HEIGHT }, containerStyle]}>
        <View style={{ width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center' }}>
          ...
          {lineStyles.map((style, index) => (
            <Animated.View key={`line-${index}`} style={style} />
          ))}
        </View>
      </Animated.View>
    </YStack>
